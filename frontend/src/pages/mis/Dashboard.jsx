...
const MisDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Build counts from MIS tickets only to override any mixed totals
  ...
  // Live refresh for counts and branch cards
  ...
  const getStatusColor = (status) => {
    const colors = {
      ...
      approved: "bg-sky-100 text-sky-700 dark:bg-sky-900/50 dark:text-sky-300",
      rejected: "bg-rose-100 text-rose-700 dark:bg-rose-900/50 dark:text-rose-300",
    };
    return colors[status?.toLowerCase()] || "bg-slate-100 text-slate-600 dark:bg-slate-700/50 dark:text-slate-300";
  };

  const statCards = [
    { label: "Total MIS Tickets", value: stats.total, color: "bg-blue-500", status: "all" },
