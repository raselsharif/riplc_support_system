            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="px-2 py-1 rounded-lg text-xs font-medium uppercase bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300">
                    MIS
                  </span>
                </div>
          ...
          {approvals.length > 0 && (
            ...
            >
              ...
              <div className="relative">
                <div className="absolute left-4 top-0 bottom-0 w-0.5" style={{ backgroundColor: "var(--border-default)" }} />
                <div className="space-y-4">
                  {approvals.map((appr) => (
                    <div key={appr.id} className="relative flex gap-4">
                      <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center ${
                        appr.action === "approved" ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-200" :
                        appr.action === "rejected" ? "bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-200" :
                      ...
                      <div className={`flex-1 p-4 rounded-xl border ${
                        appr.action === "approved" ? "bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-400/60" :
                        appr.action === "rejected" ? "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-400/60" :
                        "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-400/60"
                      }`}>
                        <div className="flex justify-between items-start">
                          <div>
                            <span className={`font-semibold capitalize ${
                              appr.action === "approved" ? "text-emerald-700 dark:text-emerald-200" :
                              appr.action === "rejected" ? "text-red-700 dark:text-red-200" :
                              "text-amber-700 dark:text-amber-200"
                            }`}>{appr.action}</span>
                            <span className="text-[var(--text-muted)]"> by </span>
                            <span className="font-semibold text-[var(--text-primary)]">{appr.approver_name}</span>
                          </div>
                          <span className="text-xs px-2 py-1 rounded-full" style={{ backgroundColor: "var(--bg-muted)", color: "var(--text-muted)" }}>
                            {format(new Date(appr.created_at), "dd MMM yyyy HH:mm")}
                          </span>
                        </div>
                        <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>{appr.department_name}</p>
                        {appr.remarks && (
