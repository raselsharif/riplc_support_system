...
const ITUsers = () => {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    if (searchTerm) {
      ...
      return user.name.toLowerCase().includes(term) || user.username.toLowerCase().includes(term) || (user.email && user.email.toLowerCase().includes(term));
    }
    return true;
  });
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>Search and manage system users</p>
          </div>
        </div>
        <button
              ...
              <table className="w-full">
                <thead style={{ backgroundColor: "var(--table-header-bg)" }}>
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase" style={{ color: "var(--text-secondary)" }}>Branch</th>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 rounded-lg text-xs font-medium ${
                            ...
                            user.role === 'user' ? 'bg-slate-100 text-slate-700 dark:bg-slate-900/50 dark:text-slate-300' :
                            ...
                            user.role === 'it' ? 'bg-sky-100 text-sky-700 dark:bg-sky-900/50 dark:text-sky-300' :
                            'bg-slate-100 text-slate-700 dark:bg-slate-900/50 dark:text-slate-300'
                          }`}>
                            {user.role}
                          </span>
                        </td>
                        <td className="px-6 py-4" style={{ color: "var(--text-primary)" }}>{user.branch_name || '-'}</td>
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 rounded-lg text-xs font-medium ${
                        ...
                        <td className="px-6 py-4">
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all hover:opacity-80 ${
                              user.is_active ? 'bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/30 dark:text-red-300' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-300'
                            }`}
                          >
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
        <div className="p-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-primary)" }}>Assigned Branches</label>
              <div className="border-2 rounded-xl p-3 max-h-40 overflow-y-auto" style={{ borderColor: "var(--border-default)" }}>
                {branches.map((b) => (
                  <label key={b.id} className="flex items-center gap-2 py-1.5 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700 rounded px-2">
                    <input
                      type="checkbox"
